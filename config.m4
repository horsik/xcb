...
if test "$PHP_XCB" = "yes"; then
    ...
    PHP_SUBST(XCB_SHARED_LIBADD)
    AC_SEARCH_LIBS([xcb_xinerama_get_screen_count], [xcb-xinerama], [
        PHP_ADD_LIBRARY_WITH_PATH(xcb-xinerama, /usr/lib, XCB_SHARED_LIBADD)
        AC_DEFINE(HAVE_LIBXCB_XINERAMA, 1, [ ])
    ], [ AC_MSG_ERROR([xcb-xinerama library not found]) ])
